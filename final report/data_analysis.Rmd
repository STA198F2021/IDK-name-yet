...
```{r residuals-access-smoking_all_int, warning = FALSE, message= FALSE}
...
```{r graph-regression-access-smoking-all-int, warning= FALSE, message= FALSE}
data_500_cities %>%
...
```{r residuals-access-binge-drinking-all-int, warning = FALSE, message= FALSE}
...
```{r residuals-access-physical-activity-all-int, warning = FALSE, message= FALSE}
...
There does seem to be a significant pattern in the residual model, so a linear model does not seem appropriate. Try a logistical model here?
...
```{r residuals-access-diabetes-all-int, warning = FALSE, message= FALSE}
...
```{r residuals-access-kidney-disease-all-int, warning = FALSE, message= FALSE}
...
There seems to be a significant pattern in the residual plot, so a linear model would not be appropriate. Try a logistic model?

## ANOVA Testing

### Map Visualization
```{r map-vis, warning= FALSE, message= FALSE}
theme_set(theme_bw())
...
```{r map-vis-insurance, warning= FALSE, message= FALSE}
states %>%
...
```{r map-vis-doctor-visits, warning = FALSE, message= FALSE}
states %>%
  left_join(data_500_cities, by = "StateDesc") -> test
data_500_cities %>%
  group_by(StateDesc) %>%
...
```{r map-vis-medicine, warning= FALSE, message= FALSE}
states %>%
  left_join(data_500_cities, by = "StateDesc") -> test
data_500_cities %>%
  group_by(StateDesc) %>%
...
```{r map-vis-smoking, warning= FALSE, message= FALSE}
states %>%
  left_join(data_500_cities, by = "StateDesc") -> test
data_500_cities %>%
  group_by(StateDesc) %>%
...
```{r map-vis-binge-drinking, warning= FALSE, message= FALSE}
states %>%
  left_join(data_500_cities, by = "StateDesc") -> test
data_500_cities %>%
  group_by(StateDesc) %>%
...
```{r map-vis-physical-activity, warning = FALSE, message= FALSE}
states %>%
  left_join(data_500_cities, by = "StateDesc") -> test
data_500_cities %>%
  group_by(StateDesc) %>%
...
```{r map-vis-heart-disease, warning= FALSE, message= FALSE}
states %>%
  left_join(data_500_cities, by = "StateDesc") -> test
data_500_cities %>%
  group_by(StateDesc) %>%
...
```{r map-vis-diabetes, warning= FALSE, Message= FALSE}
states %>%
  left_join(data_500_cities, by = "StateDesc") -> test
data_500_cities %>%
  group_by(StateDesc) %>%
...
```{r map-vis-kidney-disease, warning= FALSE, message= FALSE}
states %>%
  left_join(data_500_cities, by = "StateDesc") -> test
data_500_cities %>%
  group_by(StateDesc) %>%
...
```{r twenty-observations, warning= FALSE, message= FALSE}
data_500_cities %>%
  group_by(StateDesc) %>%
  summarize(n = n()) %>%
  print(n = 51)
...
```{r filter-ANOVA, warning= FALSE, message= FALSE}
...
```{r checking-ANOVA-assumptions-insurance, warning= FALSE, message= FALSE}
ANOVA_data_500_cities %>%
  ggplot(aes(x = insurance)) +
    geom_histogram() +
    facet_wrap(~StateDesc) +
    labs(x = "% Lack of Insurance", y = NULL, title = "Distribution of % Lack of Insurance Grouped By State")

ANOVA_data_500_cities %>%
  ggplot(aes(x = linsruance)) +
    geom_histogram() +
    facet_wrap(~StateDesc) +
    labs(x = "Log % Lack of Insurance", y = NULL, title = "Distribution of Log % Lack of Insurance Grouped By State")
```

```{r checking-ANOVA-assumptions-visits-to-doctor, warning= FALSE, message= FALSE}
ANOVA_data_500_cities %>%
...
```{r checking-ANOVA-assumptions-medicine-high-bp, warning= FALSE, message= FALSE}
ANOVA_data_500_cities %>%
...
ANOVA_data_500_cities %>%
  ggplot(aes(x = lmedicine_high_bp)) +
    geom_histogram() +
    facet_wrap(~StateDesc) +
    labs(x = "% Log Taking BP Meds", y = NULL, title = "Distribution of Log % Taking BP Meds Grouped By State")
```

```{r checking-ANOVA-assumptions-heart-disease, warning= FALSE, message= FALSE}
ANOVA_data_500_cities %>%
  ggplot(aes(x = heart_disease)) +
    geom_histogram() +
    facet_wrap(~StateDesc) +
    labs(x = "% with Heart Disease", y = NULL, title = "Distribution of % with Heart Disease Grouped By State")

ANOVA_data_500_cities %>%
  ggplot(aes(x = lheart_disease)) +
    geom_histogram() +
    facet_wrap(~StateDesc) +
    labs(x = "Log % with Heart Disease", y = NULL, title = "Distribution of % Log with Heart Disease Grouped By State")
```

```{r checking-ANOVA-assumptions-diabetes, warning= FALSE, message= FALSE}
ANOVA_data_500_cities %>%
...
ANOVA_data_500_cities %>%
  ggplot(aes(x = ldiabetes)) +
    geom_histogram() +
    facet_wrap(~StateDesc) +
    labs(x = "Log % with Diabetes", y = NULL, title = "Distribution of Log % with Diabetes Grouped By State")
```

```{r checking-ANOVA-assumptions-kidney disease, warning= FALSE, message= FALSE}
ANOVA_data_500_cities %>%
...
ANOVA_data_500_cities %>%
  ggplot(aes(x = lkidney_disease)) +
    geom_histogram() +
    facet_wrap(~StateDesc) +
    labs(x = "Log % with Kidney Disease", y = NULL, title = "Distribution of Log % with Kidney Disease Grouped By State")
```

Based on the visuals above, the variables with the most normal distribution and variance within state groups are insurance, visits_to_doctor, lmedicine_high_bp, and lheartdisease. These will be the variables examined in the ANOVA testing. Since it is unlikely that there is independence in these tests, we will perform a Bonferroni Correction for the step down tests, where there are 78 tests being conducted at the same time (13 choose 2). The new significance level is 0.05/78 = 0.000641025641

## Overall Tests

```{r ANOVA-overall-insurance, warning= FALSE, message= FALSE}
summary(aov(insurance~StateDesc,data=ANOVA_data_500_cities)) %>%
  print()
```

```{r ANOVA-overall-visits-to-doctor, warning= FALSE, message= FALSE}
summary(aov(visits_to_doctor~StateDesc,data=ANOVA_data_500_cities)) %>%
  print()
```

```{r ANOVA-overall-lmedicine_high_bp, warning= FALSE, message= FALSE}
summary(aov(lmedicine_high_bp~StateDesc,data=ANOVA_data_500_cities)) %>%
  print()
```

```{r ANOVA-overall-lheart-disease, warning= FALSE, message= FALSE}
...
```{r ANOVA-stepdown-insurance, warning= FALSE, message= FALSE}
insurancepair <- pairwise.t.test(ANOVA_data_500_cities$insurance, ANOVA_data_500_cities$StateDesc, p.adj = "holm")
...
```{r ANOVA-stepdown-visits-to-doctor, warning= FALSE, message= FALSE}
visits_to_doctor_pair <- pairwise.t.test(ANOVA_data_500_cities$visits_to_doctor, ANOVA_data_500_cities$StateDesc, p.adj = "holm")
...
```{r ANOVA-stepdown-lmedicine-high-bp, warning= FALSE, message= FALSE}
lmedicine_high_bp_pair <- pairwise.t.test(ANOVA_data_500_cities$lmedicine_high_bp, ANOVA_data_500_cities$StateDesc, p.adj = "holm")
...
```{r ANOVA-stepdown-lheart-disease, warning= FALSE, message= FALSE}
lheart_disease_pair <- pairwise.t.test(ANOVA_data_500_cities$lheart_disease, ANOVA_data_500_cities$StateDesc, p.adj = "holm")
