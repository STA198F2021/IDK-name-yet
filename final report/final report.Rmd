...
We conducted ANOVA testing to determine if there was variance within and between states for the healthcare access, behavior, and health outcome variables. After first checking for ANOVA test assumptions, we determine that state distributions of % lack of insurance, % visiting the doctor, log % with heart disease, and log % taking high BP medications had normal distributions within groups and homoscedastic variance. Since it is highly unlikely that the samples are independent, we employed a Bonferroni correction with a subsequent significance level of p = 0.000641025641. 
...
State Distribution of People Lacking Insurance
```{r ANOVA-overall-insurance, echo= FALSE, warning= FALSE, message= FALSE}
summary(aov(insurance~StateDesc,data=ANOVA_data_500_cities)) %>%
  print()
```

State Distribution of People Visiting Doctor
```{r ANOVA-overall-visits-to-doctor, echo= FALSE, warning= FALSE, message= FALSE}
summary(aov(visits_to_doctor~StateDesc,data=ANOVA_data_500_cities)) %>%
  print()
```

State Distribution of People Taking High BP Meds
```{r ANOVA-overall-lmedicine_high_bp, echo= FALSE, warning= FALSE, message= FALSE}
summary(aov(lmedicine_high_bp~StateDesc,data=ANOVA_data_500_cities)) %>%
  print()
```

State Distribution of People with Heart Disease
```{r ANOVA-overall-lheart-disease, echo= FALSE, warning= FALSE, message= FALSE}
